...
export class StringSelector{
 ...
 update(hz,notes,current,confirmedChange=false){
  const distances=notes.map(note=>Math.abs(centsBetween(hz,frequency(note))));
  const next=distances.indexOf(Math.min(...distances));
  if(next===current||distances[current]-distances[next]<35){this.reset();return current;}
  ...
  if(this.count<2)return current;
