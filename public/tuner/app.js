...
function renderStrings(){
 ...
 button.style.setProperty('--thickness',(guitar?(1.15+4.35*(notes().length-1-index)/Math.max(1,notes().length-1)):Math.max(.8,Math.min(3.55,2.1*Math.sqrt(110/frequency(note)))))+'px');
...
document.querySelectorAll('main > .tune-reminder, main > .help, main > details').forEach(item=>moreBody.append(item));const copyright=document.createElement('p');copyright.className='more-copyright';copyright.textContent='© 2026 Anthem Music. All rights reserved.';const version=document.createElement('p');version.className='app-version';version.textContent='Version 36';moreBody.append(copyright,version);document.querySelector('main').append(more);
renderInstrument();
