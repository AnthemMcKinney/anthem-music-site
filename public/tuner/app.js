...
function renderStrings(){
 ...
 const thickness=instrument.id==='banjo'&&number===5?1.15:1.15+4.35*(number-1)/Math.max(1,notes().length-1);
...
document.querySelectorAll('main > .tune-reminder, main > .help, main > details').forEach(item=>moreBody.append(item));const copyright=document.createElement('p');copyright.className='more-copyright';copyright.textContent='© 2026 Anthem Music. All rights reserved.';const version=document.createElement('p');version.className='app-version';version.textContent='Version 37';moreBody.append(copyright,version);document.querySelector('main').append(more);
renderInstrument();
